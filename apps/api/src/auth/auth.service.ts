...
export class AuthService {
  ...
  constructor(private prisma: PrismaService) {
    ...
    if (!config.url) {
      console.warn('Supabase URL not configured - using mock auth service for development')
      this.supabaseAdmin = null
    } else if (!config.serviceRoleKey) {
      console.warn('Supabase service role key not configured - limited auth functionality')
      this.supabaseAdmin = null
    } else {
  ...
  async updateProfile(userId: string, data: UpdateProfileData): Promise<AuthUser> {
    try {
