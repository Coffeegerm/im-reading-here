...
export function normalizeWorkDetails(work: OpenLibraryWork): NormalizedBookHit {
  ...
  const authors = work.authors?.map(authorRef => {
...
export function normalizeSearchDoc(doc: OpenLibraryDoc): NormalizedBookHit {
  ...
  const authorNames: string[] = Array.isArray(doc.author_name) ? doc.author_name : [];
  const authorKeys: string[] = Array.isArray(doc.author_key) ? doc.author_key : [];
  ...
  // Cover URLs
  const coverId: number | undefined = typeof doc.cover_i === 'number' ? doc.cover_i : undefined;
  const coverUrl = coverId
    ? `https://covers.openlibrary.org/b/id/${coverId}-M.jpg`
    : editionKey
  ...
  // Links
  const olWorkUrl = `https://openlibrary.org${workKey}`;
  const olEditionUrl = editionKey ? `https://openlibrary.org/books/${editionKey}` : undefined;
  const coverLinks = coverId || editionKey
    ? {
        S: coverId
  ...
  return {
    ...
    ratingsAverage: typeof doc.ratings_average === 'number' ? doc.ratings_average : undefined,
    ratingsCount: typeof doc.ratings_count === 'number' ? doc.ratings_count : undefined,
    hasFullText: Boolean(doc.has_fulltext),
    ebookCount: typeof doc.ebook_count_i === 'number' ? doc.ebook_count_i : undefined,
    publicScan: Boolean(doc.public_scan_b),
    numberOfPagesMedian: typeof doc.number_of_pages_median === 'number'
